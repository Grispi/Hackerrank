...
class Solution:
    ...
    def levelOrder(self, root):
        ...
        if root is None:
            return
        enqueueList = []
        enqueueList.append(root)
        printList = []
        while(len(enqueueList) > 0):
            printList.append(enqueueList[0].data)
            node = enqueueList.pop(0)
            if node.left is not None:
                enqueueList.append(node.left)
            if node.right is not None:
                enqueueList.append(node.right)
        print(*printList)
